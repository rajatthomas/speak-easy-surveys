...
export async function createVAD(cb: VADCallbacks, opts: VADOptions = {}) {
  try {
    const vad = await MicVAD.new({
      ...
      negativeSpeechThreshold: opts.negativeSpeechThreshold ?? 0.28,
      minSpeechMs: opts.minSpeechMs ?? 400,
      redemptionMs: opts.redemptionMs ?? 850,
      preSpeechPadMs: opts.preSpeechPadMs ?? 400,
      onSpeechStart: () => {
      ...
      onSpeechEnd: (audio: Float32Array) => {
        const wavBuf = utils.encodeWAV(audio, 1, 16000, 1, 16);
